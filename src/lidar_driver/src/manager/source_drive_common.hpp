...
public:
    ...
    bool GetDriveYamlParam(const YAML::Node& config, DriverParam &driver_param)
    {
        ...
        if (source_type == 1) {
            YamlRead<std::string>(driver_config["lidar_udp_type"], "device_ip_address",       driver_param.input_param.device_ip_address, "");
            YamlRead<uint16_t>(   driver_config["lidar_udp_type"], "udp_port",                driver_param.input_param.udp_port, 0);
            YamlRead<uint16_t>(   driver_config["lidar_udp_type"], "ptc_port",                driver_param.input_param.ptc_port, 0);
        ...
        // Resolve relative correction/firetimes paths against PROJECT_PATH
        // (package source root, baked in at compile time). This keeps
        // config.yaml portable across machines without hardcoded home paths.
        if (!driver_param.input_param.correction_file_path.empty() &&
            driver_param.input_param.correction_file_path[0] != '/') {
            driver_param.input_param.correction_file_path =
                std::string(PROJECT_PATH) + "/" + driver_param.input_param.correction_file_path;
        }
        if (!driver_param.input_param.firetimes_path.empty() &&
            driver_param.input_param.firetimes_path[0] != '/') {
            driver_param.input_param.firetimes_path =
                std::string(PROJECT_PATH) + "/" + driver_param.input_param.firetimes_path;
        }
        return true;
    }
