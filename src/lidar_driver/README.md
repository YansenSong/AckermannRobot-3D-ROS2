...
- ros1
    ...
    Create an `src` folder, copy the source code of the ros driver into it, and then run the following command in the directory where `src` is located:
        ...
        roslaunch lidar_driver start.launch
...
- ros2
    ...
    For ROS2-Dashing     
        ...
        ros2 launch lidar_driver dashing_start.py
...
### CMake library targets (`lidar_driver` dependencies)

These are the SDK CMake targets linked by `lidar_driver`. **Installable** means the target participates in `cmake --install` / the `install` space as a built artifact (ROS 2 / `colcon`); **INTERFACE** targets are header/usage-only aggregates and are not installed as separate libraries. On ROS 1 / catkin, only `lidar_driver_node` is installed: catkin does not allow installing targets defined in `add_subdirectory`, and the static libraries are already linked into the node.
