...
UART protocol, and sends it as a UDP datagram to the motion control board.
...
Protocol: Chapter 3 of 普通车运控项目通信协议说明_V1.0
  ...
  - EPS_RAW (int32 BE): steering angle / 0.1° × 10000
  - SEB_MODE, SEB_VALUE, Light, Counter
  - Checksum: uint32 BE sum of bytes 0-21

Steering: Ackermann model
  δ = atan2(ω × wheelbase, |v| + ε)
"""
...
class VehicleBridgeNode(Node):
    """ROS2 node bridging /cmd_vel to the STM32 motion control board via UDP."""
    ...
    def __init__(self):
        ...
        self._max_steer_deg: float = self.get_parameter('max_steer_deg').value
        ...
        # Steering limits in EPS raw units
        self._max_eps_raw = int(self._max_steer_deg / self.EPS_DEG_PER_RAW)
    ...
    def _timer_callback(self):
        ...
        if self._last_cmd_time is None:
            ...
            if dt > self._command_timeout:
                if self._counter % 20 == 0:  # throttle warning to ~1 Hz
                    self.get_logger().warn(
                        f'cmd_vel timeout ({dt:.2f}s > {self._command_timeout}s), '
        ...
        # Clamp speed
        v = max(self._max_reverse_speed, min(self._max_speed, v))

        # Ackermann steering conversion
        eps_raw = self._compute_eps_raw(v, omega)
    ...
    def _compute_eps_raw(self, v: float, omega: float) -> int:
        """
        Convert linear velocity and angular velocity to EPS raw steering value.

        Ackermann model:
            δ = atan2(ω × L, |v| + ε)

        Returns EPS raw: angle in units of 0.1° (e.g., 100 = 10°)
        """
        if abs(v) < self._speed_deadzone:
            return 0

        epsilon = 1e-6
        delta_rad = math.atan2(omega * self._wheelbase, abs(v) + epsilon)
        delta_deg = math.degrees(delta_rad)

        # Clamp to configured max steering
        delta_deg = max(-self._max_steer_deg, min(self._max_steer_deg, delta_deg))

        # Convert to EPS raw: degrees / 0.1° per unit
        eps_raw = int(round(delta_deg / self.EPS_DEG_PER_RAW))
    ...
    def _build_frame(self, v: float, eps_raw: int) -> bytes:
        ...
        Frame layout (all multi-byte integers big-endian):
            ...
            13-16   EPS_RAW     int32: eps_raw × 10000
