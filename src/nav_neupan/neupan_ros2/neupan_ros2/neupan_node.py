...
class NeupanCore(Node):
    ...
    def path_callback(self, path: Path) -> None:
        ...
        if self.include_initial_path_direction:
            ...
            # Smac REEDS_SHEPP keeps vehicle heading in pose.orientation,
            # while the segment direction describes the direction of travel.
            # Their relative direction determines the gear for NeuPAN.
            gears = np.ones(n_poses)
            if n_poses > 1:
                segment_directions = np.arctan2(
                    np.diff(ys), np.diff(xs)
                )
                for i in range(n_poses - 1):
                    if np.cos(thetas[i] - segment_directions[i]) < 0:
                        gears[i] = -1
                gears[-1] = gears[-2]
        else:
            self.get_logger().debug(
            ...
            # Without pose orientations, reverse segments cannot be inferred.
            gears = np.ones(n_poses)
        ...
        with self._state_lock:
            if (self.neupan_planner.initial_path is None
                ...
                # A completed path leaves both the core and wrapper in an
                # arrived state.  A new Smac path is a new tracking task, so
                # clear those terminal flags after installing the path.
