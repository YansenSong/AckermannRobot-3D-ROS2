...
public:
    ...
    rclcpp::Time lidarOdomStamp;
    bool hasLidarOdometry = false;
    deque<nav_msgs::msg::Odometry> imuOdomQueue;

    bool updateLidarToBaselinkTransform()
    {
        ...
        try
        {
            ...
            lidar2BaselinkReady = true;
            RCLCPP_INFO(get_logger(), "Resolved static transform %s -> %s",
    ...
    void lidarOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        lidarOdomStamp = rclcpp::Time(odomMsg->header.stamp);
        hasLidarOdometry = true;
    }
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        if (!hasLidarOdometry || !updateLidarToBaselinkTransform())
            return;

        // Drop IMU odometry that predates the latest scan-to-map correction.
        while (!imuOdomQueue.empty() &&
               rclcpp::Time(imuOdomQueue.front().header.stamp) <= lidarOdomStamp)
        {
        ...
        // static tf: use the original IMU odometry message stamp below.  Do
        // not rebuild a ROS timestamp through a uint32_t nanosecond value.
        ...
        // The scan-to-map pose is in lidarFrame.  Apply the cached fixed
        // lidar-to-base transform before publishing the public base pose.
        ...
        laserOdometry.pose.pose.position.x = tCur.getOrigin().x();
        laserOdometry.pose.pose.position.y = tCur.getOrigin().y();
        laserOdometry.pose.pose.position.z = tCur.getOrigin().z();
        geometry_msgs::msg::Quaternion quat_msg;
        tf2::convert(tCur.getRotation(), quat_msg);
        laserOdometry.pose.pose.orientation = quat_msg;
        pubImuOdometry->publish(laserOdometry);

        // Publish the same base pose as TF, with the native IMU odometry stamp.
        tf2::Stamped<tf2::Transform> temp_odom_to_base(
            tCur,
            tf2_ros::fromRclcpp(rclcpp::Time(laserOdometry.header.stamp)),
            odometryFrame);
        ...
        if (imuTime - last_path_time > 0.1)
        {
            ...
            while(!imuPath.poses.empty() &&
                  ROS_TIME(imuPath.poses.front().header.stamp) < lidarOdomStamp.seconds() - 1.0)
