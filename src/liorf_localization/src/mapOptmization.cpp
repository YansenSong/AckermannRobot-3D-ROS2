...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("liorf_localization_mapOptimization", options)
    {
        ...
        auto globalMapQos = rclcpp::QoS(rclcpp::KeepLast(1));
        globalMapQos.reliable();
        globalMapQos.transient_local();
        pubGlobalMap = create_publisher<sensor_msgs::msg::PointCloud2>("liorf_localization/localization/global_map", globalMapQos);
    ...
    void loadGlobalMap()
    {
        std::string global_map = globalmapPcd;
        if (global_map.empty())
        {
            const char *home = std::getenv("HOME");
            if (home == nullptr)
            {
                RCLCPP_ERROR(get_logger(), "globalmap_pcd is empty and HOME is not set; cannot load GlobalMap.pcd");
                return;
            }

            global_map = std::string(home) + savePCDDirectory;
            if (global_map.empty() || global_map.back() != '/')
                global_map += '/';
            global_map += "GlobalMap.pcd";
        }

        const int load_result = pcl::io::loadPCDFile<PointType>(global_map, *laserCloudSurfFromMap);
        if (load_result < 0)
        {
            RCLCPP_ERROR(get_logger(), "Failed to load global map '%s' (PCL error %d)", global_map.c_str(), load_result);
            return;
        }
        ...
        RCLCPP_INFO(get_logger(), "Loaded global map '%s': %d points after downsampling", global_map.c_str(), laserCloudSurfFromMapDSNum);

        if (laserCloudSurfFromMapDSNum < 1000)
        {
          RCLCPP_ERROR(get_logger(), "Global map '%s' has too few usable points (%d); localization will wait", global_map.c_str(), laserCloudSurfFromMapDSNum);
          return;
        }
        ...
        pcl::toROSMsg(*laserCloudSurfFromMapDS, global_map_msg);
        global_map_msg.header.stamp = rclcpp::Time(0);
        global_map_msg.header.frame_id = mapFrame;
    ...
    bool systemInitialize()
    {
        ...
        if(!has_initialize_pose)
        {
          RCLCPP_WARN_THROTTLE(
            ...
            "Waiting for an initial pose from RViz on /initialpose.");
          return false;
        }
        ...
        *cloudOut += *transformPointCloud(laserCloudSurfLast, &thisPose6D);
        ...
        {
            RCLCPP_INFO(get_logger(), "Initial pose ICP alignment succeeded");
            system_initialized = true;
            return true;
        } 
        else
        {
            RCLCPP_ERROR(get_logger(), "Initial pose ICP alignment failed");
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    void updatePath(const PointTypePose& pose_in)
    {
        ...
        rclcpp::Time t(
            static_cast<int64_t>(std::llround(pose_in.time * 1e9)),
            RCL_ROS_TIME);
...
int main(int argc, char** argv)
{
    ...
    // A transient-local publisher cannot be used with rclcpp intra-process
    // communication.  Keep map publication durable for late RViz subscribers.
