...
PoseEstimator::PoseEstimator(pcl::Registration<PointT, PointT>::Ptr& registration, const rclcpp::Time& stamp, const Eigen::Vector3f& pos, const Eigen::Quaternionf& quat, double cool_time_duration)
  ...
  process_noise.middleRows(0, 3) *= 0.05;
  process_noise.middleRows(3, 3) *= 0.1;
  process_noise.middleRows(6, 4) *= 0.01;
  ...
  Eigen::MatrixXf cov = Eigen::MatrixXf::Identity(16, 16) * 0.05;
...
double PoseEstimator::get_cov_trace() const {
  return static_cast<double>(ukf->cov.trace());
}
