...
def generate_launch_description():
    ...
    # 1. 解析 URDF (XACRO)
    xacro_file = os.path.join(simulation_share, 'xacro', 'robot.xacro')
    robot_description_content = ParameterValue(
        Command(['xacro ', xacro_file]),
        value_type=str
    )

    world_file_path = os.path.join(simulation_share, 'worlds', 'mini', 'mini.world')

    # 设置 GAZEBO_MODEL_PATH 环境变量
    # World models live in this package; keep the package parent for model:// lookups.
    pkg_share_env = os.pathsep.join((os.path.join(simulation_share, 'models'), os.path.dirname(simulation_share)))
    ...
    # ================= RViz =================
    rviz_config_file = os.path.join(simulation_share, 'rviz', 'view_robot.rviz')
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
