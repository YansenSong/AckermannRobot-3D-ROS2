...
class PAN(torch.nn.Module):
    ...
    def point_state_transform(self, state: torch.Tensor, obs_points: torch.Tensor):
        ...
        trans = state[0:2]
        theta = state[2, 0]
        # Build the rotation matrix with torch operations.  Constructing it
        # through torch.tensor(list_of_tensors) detaches values and emits a
        # warning when theta requires gradients.
        cos_theta = torch.cos(theta)
        sin_theta = torch.sin(theta)
        R = to_device(torch.stack((
