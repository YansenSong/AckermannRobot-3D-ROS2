...
class InitialPath:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Search window for the nearest point on the active curve.  A value
        # <= 0 means "the whole remaining curve".  The old default of 10
        # points was too small for a ROS planner whose solve time can be
        # longer than one control period: the vehicle could move past the
        # window and the reference point would remain behind it.
    ...
    def closest_point(self, state, threshold=0.1, ind_range=10):
        ...
        if ind_range is None or ind_range <= 0:
            end = len(self.cur_curve)
        else:
