...
class neupan(torch.nn.Module):
    ...
    def __init__(
        ...
        avoidance_seed_enabled: bool = False,
        avoidance_seed_distance: float = 1.2,
        avoidance_seed_clearance: float = 0.15,
        avoidance_seed_steer: float = 0.25,
        **kwargs,
    ) -> None:
        ...
        self.avoidance_seed_enabled = avoidance_seed_enabled
        self.avoidance_seed_distance = float(avoidance_seed_distance)
        self.avoidance_seed_clearance = float(avoidance_seed_clearance)
        self.avoidance_seed_steer = float(avoidance_seed_steer)
        self.avoidance_side = 0
        self._avoidance_clear_cycles = 0
    ...
    def init_from_yaml(cls, yaml_file, **kwargs):
        ...
        with open(abs_path, "r") as f:
            ...
            # Merge nested overrides instead of replacing an entire section.
            # The ROS wrapper overrides only pan.dune_checkpoint; a shallow
            # update would silently discard pan.iter_num and the configured
            # dune/nrmp point budgets and make PAN use its defaults.
    ...
    def forward(self, state, points, velocities=None):
        ...
        avoidance_seeded = self._seed_avoidance_nominal(
            state, points, nom_input_np
        )
        ...
        self.info["avoidance_seeded"] = avoidance_seeded
        self.info["avoidance_side"] = self.avoidance_side
    ...
    def _seed_avoidance_nominal(self, state, points, nom_input_np):
        """Break the left/right local minimum for a path-blocking obstacle.

        A straight Ackermann nominal trajectory has no first-order lateral
        response to steering.  When an obstacle is centred on that nominal
        path, the first convexification can therefore prefer stopping or
        reversing over choosing either homotopy.  Seed a small, dynamically
        feasible S-turn and let PAN optimize it.  The selected side is kept
        briefly so scan noise cannot flip the solution every cycle.
        """
        if (
            not self.avoidance_seed_enabled
            or points is None
            or self.robot.kinematics != "acker"
            or points.shape[1] == 0
        ):
            return False

        # Do not alter intentional reverse sections from a Reeds-Shepp path.
        if self.ipath.cur_point is not None and self.ipath.cur_point[-1, 0] < 0:
            return False

        robot_state = np.asarray(state[:3], dtype=float).reshape(3, 1)
        obstacle_points = np.asarray(points[:2], dtype=float)
        translation, rotation = get_transform(robot_state)
        local_points = rotation.T @ (obstacle_points - translation)

        vertices = np.asarray(self.robot.vertices, dtype=float)
        front_x = float(np.max(vertices[0]))
        half_width = 0.5 * float(np.max(vertices[1]) - np.min(vertices[1]))
        corridor_half_width = half_width + self.avoidance_seed_clearance

        forward_mask = (
            (local_points[0] > front_x)
            & (local_points[0] < front_x + self.avoidance_seed_distance)
        )
        blocking_mask = forward_mask & (
            np.abs(local_points[1]) < corridor_half_width
        )

        if not np.any(blocking_mask):
            self._avoidance_clear_cycles += 1
            if self._avoidance_clear_cycles >= 3:
                self.avoidance_side = 0
            return False

        self._avoidance_clear_cycles = 0
        if self.avoidance_side == 0:
            # Prefer the side with fewer nearby returns.  A deterministic
            # left preference resolves a perfectly symmetric tie.
            nearby_y = local_points[1, forward_mask]
            left_count = np.count_nonzero(
                (nearby_y > 0.0)
                & (nearby_y < 2.0 * corridor_half_width)
            )
            right_count = np.count_nonzero(
                (nearby_y < 0.0)
                & (nearby_y > -2.0 * corridor_half_width)
            )
            self.avoidance_side = 1 if left_count <= right_count else -1

        # One smooth steering wave moves laterally around the obstacle and
        # then starts returning towards the reference path.  Its adjacent
        # samples respect the configured steering acceleration bound.
        phase = np.linspace(0.0, 2.0 * np.pi, self.T + 2)[1:-1]
        steer_seed = (
            self.avoidance_side
            * self.avoidance_seed_steer
            * np.sin(phase)
        )
        max_steer = float(self.robot.max_speed[1, 0])
        steer_seed = np.clip(steer_seed, -max_steer, max_steer)

        nom_u = nom_input_np[1]
        positive_ref_speed = max(float(self.ref_speed), 0.0)
        nom_u[0, :] = positive_ref_speed
        nom_u[1, :] = steer_seed

        # Regenerate the nominal state from the seeded controls.  The
        # reference trajectory itself remains the global planner path.
        seeded_states = [robot_state]
        predicted_state = robot_state
        for index in range(self.T):
            predicted_state = self.ipath.motion_predict_model(
                predicted_state,
                nom_u[:, index:index + 1],
                self.robot.L,
                self.dt,
            )
            seeded_states.append(predicted_state)
        nom_input_np[0][:, :] = np.hstack(seeded_states)
        return True
