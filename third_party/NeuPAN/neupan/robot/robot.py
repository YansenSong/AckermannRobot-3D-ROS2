...
class robot:
    ...
    def __init__(
        ...
        min_speed: Optional[Union[list[float], np.ndarray]] = None,
        **kwargs,
    ):
        ...
        self.max_speed = np.asarray(max_speed, dtype=float).reshape(2, 1)
        self.max_acce = np.asarray(max_acce, dtype=float).reshape(2, 1)
        # The original implementation only enforced abs(u) <= max_speed,
        # which silently ignored the runtime YAML min_speed option because it
        # was consumed by **kwargs.  Keep the old symmetric behavior for
        # configs without min_speed, while allowing true asymmetric bounds.
        if min_speed is None:
            self.min_speed = -self.max_speed.copy()
        else:
            self.min_speed = np.asarray(min_speed, dtype=float).reshape(2, 1)

        if np.any(self.min_speed > self.max_speed):
            raise ValueError(
                f"min_speed must not exceed max_speed: "
                f"{self.min_speed.ravel()} > {self.max_speed.ravel()}"
            )
    ...
    def bound_su_constraints(self):
        ...
        constraints += [self.indep_u >= self.min_speed]
        constraints += [self.indep_u <= self.max_speed]
    ...
    def linear_ackermann_model(self, nom_st, nom_ut, dt, L):
        ...
        tan_psi = torch.tan(psi)
        cos_psi = torch.cos(psi)
        A = torch.stack([
            torch.stack([one, zero, -v * dt * sin_phi]),
            torch.stack([zero, one, v * dt * cos_phi]),
            torch.stack([zero, zero, one]),
        ])

        B = torch.stack([
            torch.stack([cos_phi * dt, zero]),
            torch.stack([sin_phi * dt, zero]),
            torch.stack([tan_psi * dt / L,
                         v * dt / (L * cos_psi ** 2)]),
        ])
